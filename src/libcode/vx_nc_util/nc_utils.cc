...
unixtime get_valid_time_from_file_path(const std::string &filepath);
unixtime get_init_time_from_file_path(const std::string &filepath);
unixtime get_time_from_TRMM_3B42_3hourly_filename(const std::string &filename);
unixtime get_time_from_TRMM_3B42_daily_filename(const std::string &filename);
...
unixtime get_init_time(NcFile *nc_file, const string &filepath) {
   ...
   if (IS_INVALID_NC(init_time_var)) {
      ...
      init_time = get_init_time_from_file_path(filepath);
      if (init_time == 0) {
         mlog << Debug(4) << method_name
         ...
         mlog << Debug(4) << method_name
              << "get InitTime (" << unix_to_yyyymmdd_hhmmss(init_time) << ") from the file name.\n";
      }
   }
   else {
      unixtime ut = 0;
...
unixtime get_time_from_file_path(const string &filepath, bool is_init_time) {
  ...
  file_time = get_time_from_TRMM_3B42_3hourly_filename(filename);
  if (file_time != 0)
    return file_time + (int)(1.5 * 3600.0);

  // See if this is a TRMM daily precip file

  file_time = get_time_from_TRMM_3B42_daily_filename(filename);
  if (file_time != 0) {
    float offset = (is_init_time ? 1.5 : 22.5);
    return file_time + (int)(offset * 3600.0);
  }
...
unixtime get_init_time_from_file_path(const string &filepath) {
  bool is_init_time = true;
  return get_time_from_file_path(filepath, is_init_time);
}


////////////////////////////////////////////////////////////////////////

unixtime get_valid_time_from_file_path(const string &filepath) {
  bool is_init_time = false;
  return get_time_from_file_path(filepath, is_init_time);
}
...
unixtime get_time_from_TRMM_3B42_3hourly_filename(const string &filename) {
...
unixtime get_time_from_TRMM_3B42_daily_filename(const string &filename) {
  // The format of the TRMM 3hourly files is:
