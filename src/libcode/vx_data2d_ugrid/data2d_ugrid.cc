...
bool MetUGridDataFile::fail_with_error(const char *method_name, const std::string &msg) {
   mlog << Error << std::string("\n") << method_name << msg << "\n\n";
   close();
   return false;
}
...
bool MetUGridDataFile::open(const char * _filename) {
   const char *method_name = "MetUGridDataFile::open(const char *) -> ";
...
bool MetUGridDataFile::open_metadata(const char * _filename) {
   ...
   if (! _file) {
      return fail_with_error(method_name, std::string("UGridFile is not initiated to handle \"") + _filename + "\"");
   }
