...
void Polyline::set_name(const char *n) {
   ...
   if(n) {
      ...
      if(!name) {
         cerr << "\n\nERROR: Polyline::set_name(const char *) -> "
...
void Polyline::extend_points(int n) {
   ...
   if(n_alloc == 0) {
      ...
      if(!u || !v) {
         cerr << "\n\nERROR: Polyline::extend_points(int) -> "
	      << "memory allocation error 1" << "\n\n" << flush;
...
void Polyline::bounding_box(BoundingBox &bb) const {
   int i;

   // Find min and max of current Polyline's points
   for(i=0; i<n_points; i++) {

      if(u[i] > bb.x_ur) bb.x_ur = u[i];
      if(u[i] < bb.x_ll) bb.x_ll = u[i];

      if(v[i] > bb.y_ur) bb.y_ur = v[i];
      if(v[i] < bb.y_ll) bb.y_ll = v[i];
