...
class SingleFeature {
   ...
   public:
     ...
     ~SingleFeature();
      ...
      void set(const WrfData &raw,  const WrfData &thresh,
               const WrfData &mask, const double p);

      const WrfData *Raw;    //  NOT allocated, so DON'T delete!
      const WrfData *Thresh; //  NOT allocated, so DON'T delete!
      const WrfData *Mask;   //  NOT allocated, so DON'T delete!
