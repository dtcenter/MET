...
////////////////////////////////////////////////////////////////////////
//
//   Filename:   gen_circle_mask.cc
//
//   Description:
//
//   Mod#   Date      Name            Description
//   ----   ----      ----            -----------
//   000    03/18/14  Halley Gotway   New
//
////////////////////////////////////////////////////////////////////////
...
// Input data file, lat/lon locations, and output NetCDF file
...
static double       radius = 200; // km

// List of lat/lon points for masking
...
void process_command_line(int argc, char **argv) {
   ...
   // the data filename, the mask filename, and the netCDF filename.
   //
   if(cline.n() != 3) usage();
...
void process_data_file() {
   ...
   // Open the input gridded data file
...
void write_netcdf() {
   ...
   for(x=0; x<grid.nx(); x++) {
      for(y=0; y<grid.ny(); y++) {
         ...
         // Loop through the polyline points
         for(i=0; i<poly_mask.n_points(); i++) {
            dist = gc_dist(lat, lon, poly_mask.lat(i), poly_mask.lon(i));
...
void usage() {
   ...
   cout << "\n*** Model Evaluation Tools (MET" << met_version
        ...
        << "\t\t\"latlon_file\" is an ASCII file containing Lat/Lon "
        << "locations (required).\n"
        ...
        << "\t\t\"-radius km\" overrides the default distance around "
        << "each Lat/Lon location (" << radius
        << " km) to be included in the mask" << " (optional).\n"
