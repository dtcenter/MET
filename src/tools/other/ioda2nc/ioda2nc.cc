...
static constexpr char DEF_CONFIG_NAME[] = "MET_BASE/config/IODA2NCConfig_default";
...
static void open_netcdf_output();
...
static void process_ioda_file(int i_pb) {
   ...
   ConcatString prefix;
   ConcatString start_time_str;
   ConcatString end_time_str;
   char     max_time_str[max_str_len];
   char     min_time_str[max_str_len];
   ...
   for(int i_read=0; i_read<npbmsg; i_read++) {

      if(mlog.verbosity_level() > 0 && bin_count > 0
         && (i_read+1)%bin_count == 0) {
      ...
      const double quality_mark)
{
   ...
   obs.setHeaderIndex(int(obs_arr[0]));
...
static bool get_meta_data_strings(NcVar &var, char *metadata_buf) {
   bool status = false;
...
static void set_ioda_files(const StringArray & a)
{
   ioda_files.add(a[0]);
}
...
static void set_logfile(const StringArray & a)
{
