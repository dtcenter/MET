...
static void plot_engine(PSfile &, ModeFuzzyEngine &, EngineType, const char *);
static void plot_threshold_merging(PSfile &, ModeFuzzyEngine &, const char *, int);

static void draw_border(PSfile &, Box &);
static void draw_map(PSfile &, Box &);
static void render_image(PSfile &, ModeFuzzyEngine &, EngineType, const ShapeData &, Box &,
                         int, int);
static void plot_simple_ids(PSfile &, ModeFuzzyEngine &, Box &, int);
static void draw_convex_hulls(PSfile &, ModeFuzzyEngine &, const Box &, int, int);
static void draw_boundaries(PSfile &, ModeFuzzyEngine &, const Box &, int);
...
void plot_engine() {
   ...
   if(engine.conf_info.fcst_merge_flag == MergeType_Both ||
      ...
      plot_engine(plot, *engine.fcst_engine, FFEng, "Forecast: ModeFuzzyEngine Merging");
   }
   ...
   if(engine.conf_info.obs_merge_flag == MergeType_Both ||
      ...
      plot_engine(plot, *engine.obs_engine, OOEng, "Observation: ModeFuzzyEngine Merging");
   }
...
void plot_engine(PSfile &p, ModeFuzzyEngine &eng, EngineType eng_type, const char *title) {
   Box dim;
...
void plot_threshold_merging(PSfile &p, ModeFuzzyEngine &eng, const char *title, int fcst) {
   Box dim;
...
void render_image(PSfile &p, ModeFuzzyEngine &eng, EngineType eng_type,
...
void plot_simple_ids(PSfile &p, ModeFuzzyEngine &eng, Box &dim, int fcst) {
   int i, n_shapes;
...
void draw_convex_hulls(PSfile &p, ModeFuzzyEngine &eng, const Box &dim,
...
void draw_boundaries(PSfile &p, ModeFuzzyEngine &eng, const Box &dim, int fcst) {
   int i, j;
