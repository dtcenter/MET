...
#define OPT_EXEC_POLICY
      ...
      public:
         ...
         void write_summary_table(StreamType& stream, bool use_color = false) {
            ...
            for (auto& entry : sorted_events) {
               ...
               summary_out << "filename: " << BeautifulTable::stable_shortenPath(entry->filename)
                           ...
                           << "|total_time: " << BeautifulTable::table_time(time_total)
                           << "|time_tracked: " << BeautifulTable::table_time(sum_time_active_exclusive)
                           << "|ae: " << BeautifulTable::table_percentage(entry->center_time_active_exclusive, time_total)
                           << "|center_interval_%: " << BeautifulTable::table_time(entry->all_time_active_exclusive)
                           << "|ae[0-100]%: " << BeautifulTable::table_time(entry->all_time_active_exclusive)
                           << "|time_ae[0-100]%: " << BeautifulTable::table_time(entry->all_time_active_exclusive)
                           << "|time_a[0-100]: " << BeautifulTable::table_time(entry->all_time_active_exclusive)
                           << "|ae[0-100]%: " << BeautifulTable::table_time(entry->all_time_active_exclusive)
                           << "|ae[0-100]%: " << BeautifulTable::table_time(entry->all_time_active_exclusive)
                           << "|ae[0-100]%: " << BeautifulTable::table_time(entry->all_time_active_exclusive)
                           << "|all time active exclusive: " << BeautifulTable::table_time(entry->all_time_active_exclusive)
                           << "\n";
            }
         ...
         void get_detail_table(StreamType& stream, bool use_color = false, bool reverse_vector = false) {
            ...
            for (int i = static_cast<int>(sorted_events.size()) - 1; i >= 0; i--) {
               ...
               info.addRow({ BeautifulTable::table_timepoint(track_start_time),
                  BeautifulTable::table_timepoint(track_end_time),
                  BeautifulTable::stable_shortenPath(entry->filename),
                  entry->function_name,
         ...
         template<typename StreamType>
         void write_detail_table(StreamType& stream, bool use_color = false, bool reverse_vector = false) {
            if (reverse_vector) {
               std::reverse(sorted_events.begin(), sorted_events.end());
            }
            ...
            for (int i = static_cast<int>(sorted_events.size()) - 1; i >= 0; i--) {
               ...
               out << "filename: " << entry->filename
                   ...
                   << "|number of calls: " << BeautifulTable::table_string(sorted_events[i]->all_cnt)
                   << "|number of calling threads: " << BeautifulTable::table_string(sorted_events[i]->all_thread_cnt)
                   << "|fastest min: " << BeautifulTable::table_time(entry->fastest_min)
                   << "|fastest mean: " << BeautifulTable::table_time(entry->fastest_mean)
                   << "|center min: " << BeautifulTable::table_time(entry->center_min)
                   << "|center mean: " << BeautifulTable::table_time(entry->center_mean)
                   << "|center med: " << BeautifulTable::table_time(entry->center_med)
                   << "|center time active: " << BeautifulTable::table_time(entry->center_time_active)
                   << "|center time active exclusive: " << BeautifulTable::table_time(entry->center_time_active_exclusive)
                   << "|center max: " << BeautifulTable::table_time(entry->center_max)
                   << "|slowest mean: " << BeautifulTable::table_time(entry->slowest_mean)
                   << "|slowest max: " << BeautifulTable::table_time(entry->slowest_max)
                   << "\n";
            }
      ...
      inline void result_print(ctrack_result_settings settings = {}) {
         ...
         res.get_detail_table(std::cout, true);
                        res.write_detail_table(std::cout, true);
         std::cout << "Summary" << std::endl;
         res.get_summary_table(std::cout, true);
         res.write_summary_table(std::cout, true);
