...
DOCKERHUB_TAG=dtcenter/met-base:unit_test
...
mount_args="-v ${GITHUB_WORKSPACE}:/${MET_REPO_DIR} -v ${LOCAL_OUTPUT_DIR}:${DOCKER_OUTPUT_DIR} -v ${LOCAL_DIFF_DIR}:${DOCKER_DIFF_DIR} -v ${LOCAL_LOG_DIR}:${DOCKER_LOG_DIR}"
...
cmd="export MET_REPO_DIR=${MET_REPO_DIR}; \${MET_REPO_DIR}/.github/jobs/run_diff_tests.sh"
