...
For a Range/Azimuth grid, the syntax is

.. code-block:: none

  rngazi azimuth_n range_n range_max_km lat_center lon_center

The parameters **lat_center** and **lon_center** define the latitude and longitude for the center of the grid. The **azimuth_n** parameter defines the number of equally-spaced azimuth values, with spacing in degrees defined as 360 / **azimuth_n**. The **range_n** and **max_range_km** parameters define the number of and maximum value of the ranges, with spacing in kilometers defined as **max_range_km** / ( **range_n** - 1 ).
