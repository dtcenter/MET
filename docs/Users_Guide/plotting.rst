...
This section describes how to check your data files using plotting utilities. Point observations can be plotted using the Plot-Point-Obs utility. A single model level can be plotted using the plot_data_plane utility. Occasionally, a post-processing or timing error can lead to errors in MET. These tools can assist the user by showing the data to be verified to ensure that times and locations match up as expected.

.. note::

   Earlier versions included the plot_mode_field utility to visualize the NetCDF output generated by the MODE tool.
   This utility was deprecated and removed for MET version 13.0.0 to reduce MET's external library dependencies.
   Users are encouraged to visualize the NetCDF output from MODE using the plot_data_plane utility and/or
   using functionality provided in `METplotpy <https://github.com/dtcenter/METplotpy>`_.
