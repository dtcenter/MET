...
6. The -input option specifies the path to an existing Series-Analysis output file. When computing statistics for the data provided, Series-Analysis intializes the partial sums (SL1L2, SAL1L2 line types) and contingency table counts (CTC, MCTC, and PCT line types) data using the input_file data.

.. note:: When the -input option is used, only statistics that are derivable from partial sums and contingency table counts can be requested.
