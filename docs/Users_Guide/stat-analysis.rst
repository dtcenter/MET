...
In earlier versions, the Stat-Analysis tool always performed a two step process when reading input data. It used the common filtering criteria defined in the top section of the configuration file to subset the input data and write the result to a temporary file, as described in :numref:`Contributor's Guide Section %s <tmp_files_stat_analysis>`. All analysis jobs were performed by reading data from that temporary file. However, this temporary file logic has been refined. A temporary file is now only written if the **jobs** entry defines more than one job and common filtering criteria is actually specified in the top section of the configuration file or on the command line. If a temporary file is not needed, each job processes all of the input data files. If it is needed, then each job processes data from that temporary file. After all jobs have run, the Stat-Analysis tool deletes the temporary file it created.

In practice, the benefit of writing a temporary file varies considerably based on the amount of input data and filtering options requested. When running multiple jobs on a small subset of the input data, grouping filtering criteria common for to all jobs in the top-level section should enable Stat-Analysis to run more efficiently. Applying the common filters once is faster than re-applying them for multiple jobs. However, if running a single job or with no common filtering criteria, writing an intermediate temporary file would provide no benefit.
