...
void check_llh_to_ecef() {
   ...
   test_llh_to_ecef(34.0522, -118.40806, 0.0, -2516.715, -4653.003, 3551.245, "LA");
   test_llh_to_ecef(-9.59874, 287.326, 100, 1873.072, -6004.143, -1056.531, "First Point");
   test_llh_to_ecef(-27.9711, 107.326, 100, -1678.837,  5381.522, -2973.724, "Middle Point");
   test_llh_to_ecef(75.6264, 287.326, 100, 473.024, -1516.283,  6156.59, "Last Point");
}
...
void llh_to_ecef(double lat, double lon, double alt_m, double *x_km, double *y_km, double *z_km) {
   ...
   *x_km = (radius * C + alt_m) * cosLat * cos(lon_r) / 1000.;
   *y_km = (radius * C + alt_m) * cosLat * sin(lon_r) / 1000.;
   *z_km = (radius * S + alt_m) * sinLat / 1000.;
}
