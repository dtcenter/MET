...
def comp_dir(truth_dir, output_dir, diff_dir='', debug=True, save_diff=True):
    ...
    if diff_files:
        ...
        for truth_file, out_file, _, diff_file in diff_files:
            if truth_file:
                copy_to_diff_dir(truth_file, '_truth', truth_dir, diff_dir)
                #data_dir = truth_dir
                #label = '_truth'
            if out_file:
                copy_to_diff_dir(out_file, '_output', output_dir, diff_dir)
                # data_dir = output_dir
                # label = '_output'
            if diff_file:
                copy_to_diff_dir(diff_file, '', output_dir, diff_dir)
                # data_dir = output_dir
                # label = ''
