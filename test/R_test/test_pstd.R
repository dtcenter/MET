...
strFileSaDump	= paste(strDirTest, "/sa_out/sa_pstd_dump_{fcst_lead}.stat", sep="");
strFileSaOut	= paste(strDirTest, "/sa_out/sa_pstd_agg_{fcst_lead}.out", sep="");
strFileSaStat	= paste(strDirTest, "/sa_out/sa_pstd_agg.stat", sep="");
strSaJob		= "aggregate_stat -out_line_type PSTD";
...
for(strFcstLead in listFcstLead){
	...
	runStatAnalysis(listSaCrit, strSaJob, strFileSaDumpLead, strFileSaOutLead, strDirLookin, 
					show=boolShowCmd);
...
strFileMvPlot = paste(strDirMvScripts, "/plot_pstd.R",    sep="");
strFileMvData = paste(strDirMvData,    "/plot_pstd.data", sep="");
rmFile(strFileMvPlot);

# run the METViewer job to generate a reliability diagram, and read the data
strCmdMvRely = paste(strMvExec, "xml/plot_pstd.xml");
