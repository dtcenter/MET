...
void SingleFeature::clear() {
   ...
   Raw    = (const WrfData *) 0;
   Thresh = (const WrfData *) 0;
   Mask   = (const WrfData *) 0;
...
void SingleFeature::set(const WrfData &raw, const WrfData &thresh,
                        const WrfData &mask, const double p) {
   int i;
   WrfData split_wd, obj_wd;
...
void PairFeature::set(const SingleFeature &fcst,
   ...
   symmetric_diff = 0.0;
   for(x=0; x<(Fcst->Mask->get_nx()); ++x) {
      for(y=0; y<(Fcst->Mask->get_ny()); ++y) {
...
ostream & operator<<(ostream & out, const SingleFeature & s) {
   WrfDataHeader h = s.Raw->get_header();
...
void get_percentiles(DistributionPercentiles &ptile,
                     const WrfData &raw, const WrfData &mask,
                     const double p) {
   ...
   for(x=0; x<nx; ++x) {
      for(y=0; y<ny; ++y) {
         ...
         if((mask.f_is_on(x, y)) &&
            (raw.is_valid_xy(x, y)) &&
            (raw.get_xy_int(x, y) > 0)) {

            v[count++] = (double) (raw.get_xy_double(x, y));
         }
      }
   }
