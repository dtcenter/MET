...
DataPlane met_regrid_bilinear (const DataPlane & from_data, const Grid & from_grid, const Grid & to_grid, const RegridInfo & info)
...
for (xt=0; xt<(to_grid.nx()); ++xt)  {
   ...
   for (yt=0; yt<(to_grid.ny()); ++yt)  {
      ...
      if ( (xf < 0) || (xf >= from_grid.nx()) || (yf < 0) || (yf >= from_grid.ny()) )  {
         ...
         value = interp_bilin(from_data, x_from, y_from);
