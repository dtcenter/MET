...
if ( Npoints < 0 )  {
   ...
   cerr << "\n\n  MaskPoly::load() -> can't determine line count in file \"" << filename << "\"\n\n";
...
if ( !in )  {
   ...
   cerr << "\n\n  MaskPoly::load() -> can't open mask poly file \"" << filename << "\"\n\n";
...
for (j=0; j<Npoints; ++j)  {
   ...
   if ( !in )  {
      ...
      cerr << "\n\n  MaskPoly::load() -> read error in mask poly file \"" << filename << "\"\n\n";
   ...
   if ( 90.0 - abs(Lat[j]) < 1.0 ) {
      ...
      cerr << "\n\n  MaskPoly::load() -> encountered latitude value too close to a pole, (Lat, Lon) = ("
...
int MaskPoly::latlon_is_inside(double lat, double lon) const
