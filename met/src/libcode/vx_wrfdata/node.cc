...
void Node::bounding_box(BoundingBox &bb) const {
   Node *n_ptr;
...
double node_dist(const Node &a, const Node &b) {
   ...
   if(num_a == 0 || num_b == 0) {
      cerr << "\n\nERROR: double node_dist(Node &, Node &) -> "
...
double node_polyline_dist(const Node &a, const Polyline &b) {
   ...
   if(num_a == 0) {
      cerr << "\n\nERROR: double node_polyline_dist(Node &, Node &) -> "
           << "encountered empty node tree\n\n" << flush;
