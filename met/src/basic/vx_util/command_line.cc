...
if ( !u )  {
   ...
   mlog << Error << "CLOptionInfoArray::extend(int) -> memory allocation error\n\n";
...
if ( n < 0 )  {
   ...
   mlog << Error << "CLOptionInfoArray::set_alloc_int(int) -> bad value ... " << n << "\n\n";
...
if ( (n < 0) || (n >= Nelements) )  {
   ...
   mlog << Error << "CLOptionInfoArray::operator[](int) -> range check error ... " << n << "\n\n";
