...
def debug_cyl_grid_dump(
    ...
    out_fname = output_filename.format(grib_var_name=grib_var_name,
                                       level_hPa=level_hPa,
                                       hour=hour)
    ...
    ds = xr.Dataset(data_vars=data_vars,
                    coords=dict(theta_radians=theta_radians,
                                radius_km=radius_km))
...
def _obtain_data_at_level(grib_dataset: xr.Dataset,
                          grib_var_name: str,
                          level_hPa: Optional[int] = None) -> np.ndarray:
...
def _convert(data: np.ndarray,
             unit_converter: Optional[str] = None) -> np.ndarray:
...
def mean_in_radius_range(
        ...
        unit_converter: str = None,
        **_kwargs) -> float:
    LOGGER.info("Started mean in radius average: %s %s", grib_var_name,
                str(level_hPa))
    ...
    lerp = cylindrical_grid_interpolator
    data_on_grid = lerp(data)
    mean = area_average(data_on_grid, lerp.rad_2d_km, min_radius_km,
                        max_radius_km)
    LOGGER.info("Finished mean in radius average: %s %s", grib_var_name,
                str(level_hPa))
    return mean


#TODO: Add unit tests, possible move to diag_lib
def area_average(data_on_grid: np.ndarray, radii_2d_km: np.ndarray,
...
def track_row_lookup(track_row: pd.DataFrame,
                     column_name: str,
                     convert_to_0_360: bool = False,
                     **_kwargs):
    LOGGER.info("Started track row lookup: %s convert_to_0_360: %d",
                column_name, convert_to_0_360)
    val = track_row[column_name][0]
    if convert_to_0_360:
        val %= 360
    LOGGER.info("Finished track row lookup: %s convert_to_0_360: %d val: %f",
                column_name, convert_to_0_360, val)
    return val


def shear(hour: int, results: hr_results.ForecastHourResults, u_name: str,
          v_name: str, bottom_hPa: float, top_hPa: float,
          **_kwargs) -> Tuple[float, float]:
    LOGGER.info(
        "Started shear calculations hour: %d u_name: %s v_name: %s bottom_hPa: %f top_hPa: %f",
        hour, u_name, v_name, bottom_hPa, top_hPa)
...
def _shear_component(hour: int, results: hr_results.ForecastHourResults,
                     var_name: str, bottom_hPa: float,
                     top_hPa: float) -> float:
...
def temperature_gradient(hour: int, results: hr_results.ForecastHourResults,
                         u_name: str, v_name: str, bottom_hPa: int,
                         top_hPa: int, tc_lat: float, **_kwargs) -> float:
    bottom_u, top_u = _component_at_levels(results, u_name, hour, bottom_hPa,
                                           top_hPa)
    bottom_v, top_v = _component_at_levels(results, v_name, hour, bottom_hPa,
                                           top_hPa)
...
def _component_at_levels(results: hr_results.ForecastHourResults,
                         var_name: str, hour: int, bottom_hPa: int,
                         top_hPa: int) -> Tuple[float, float]:
    bottom = results.soundings[var_name].sel(forecast_hour=hour,
                                             level_hPa=bottom_hPa)
    top = results.soundings[var_name].sel(forecast_hour=hour,
                                          level_hPa=top_hPa)
...
class LandLUT:
    ...
    def distance(self, lon: float, lat: float) -> float:
        ...
        w_1_1 = self._inverse_distance(lon,
                                       i_right,
                                       lat,
                                       j_up,
                                       wrap_dx=wrap_dx)
...
def read_land_lut_file(
        lut_filename: pathlib.Path) -> Tuple[np.ndarray, LUTExtents]:
...
def _get_header_info(header_line: str) -> LUTExtents:
    ...
    extents = LUTExtents(ll_lon, ll_lat, ur_lon, ur_lat, nx, ny, lon_spacing,
                         lat_spacing)
    return extents
...
def distance_to_land_lookup(land_lut: LandLUT,
                            hour: int,
    ...
    LOGGER.info(
        ...
        hour, lon_column_name, lat_column_name)
    ...
    LOGGER.info(
        ...
        distance, hour, lon_column_name, lat_column_name)
    return distance


def storm_r_theta(model_track: pd.DataFrame,
                  hour: int,
    ...
    LOGGER.info(
        ...
        "tau_column_name:%s, time_column_name:%s", hour, model_time,
        time_delta_hours, lon_column_name, lat_column_name, tau_column_name,
        time_column_name)
    ...
    lower_row = mt_rows.loc[mt_rows[tau_column_name] == hour +
                            lower_offset].iloc[0]
    upper_row = mt_rows.loc[mt_rows[tau_column_name] == hour +
                            upper_offset].iloc[0]
    ...
    LOGGER.info(
        ...
        "time_delta_hours:%d, lon_column_name:%s, lat_column_name:%s, "
        "tau_column_name:%s, time_column_name:%s", r, theta, hour, model_time,
        time_delta_hours, lon_column_name, lat_column_name, tau_column_name,
        time_column_name)
    return r, theta
...
def radial_and_tangential_area_average(
        grib_dataset: xr.Dataset,
        cylindrical_grid_interpolator: cg.CylindricalGridInterpolator,
        u_name: str, v_name: str, level_hPa: int, min_radius_km: float,
        max_radius_km: float, **_kwargs) -> Tuple[float, float]:
    ...
    radial_radius_avg = area_average(radial_azavg, radii, min_radius_km,
                                     max_radius_km)
    tan_radius_avg = area_average(tan_azavg, radii, min_radius_km,
                                  max_radius_km)
...
def divergence_vorticity(
        grib_dataset: xr.Dataset,
        cylindrical_grid_interpolator: cg.CylindricalGridInterpolator,
        u_name: str, v_name: str, level_hPa: int, radius_km: float,
        **_kwargs) -> Tuple[float]:
    LOGGER.info(
        "Started computing divergence and vorticity u_name:%s, v_name:%s, level_hPa:%d, radius_km:%f",
        u_name, v_name, level_hPa, radius_km)
    radial_azavg, tan_azavg, radii = _gen_azimuthal_avg_of_radial_tan_winds(
        grib_dataset, cylindrical_grid_interpolator, u_name, v_name, level_hPa)

    divergence, vorticity = _div_vort(radii, radius_km, radial_azavg,
                                      tan_azavg)

    LOGGER.info(
        "Finished computing divergence:%f and vorticity:%f u_name:%s, v_name:%s, level_hPa:%d, radius_km:%f",
        divergence, vorticity, u_name, v_name, level_hPa, radius_km)
    return divergence, vorticity


def _div_vort(radii: np.ndarray, radius: float, radial_azavg: np.ndarray,
              tan_azavg: np.ndarray) -> Tuple[float, float]:
...
def _nearest_radius(radii: np.ndarray,
                    desired_radius: float) -> Tuple[float, int]:
...
def _gen_azimuthal_avg_of_radial_tan_winds(
        ...
        level_hPa: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    radial_azavg, tan_azavg = _rad_tan_azavg(u_cyl, v_cyl,
                                             lerp.theta_2d_radians, 0)
...
def _rad_tan_azavg(u_cyl: np.ndarray, v_cyl: np.ndarray,
                   theta_2d_radians: np.ndarray,
                   avg_axis: int) -> Tuple[np.ndarray, np.ndarray]:
    # Convert to radial and tangential wind
    radial = u_cyl * np.cos(theta_2d_radians) + v_cyl * np.sin(
        theta_2d_radians)
    tangential = -u_cyl * np.sin(theta_2d_radians) + v_cyl * np.cos(
        theta_2d_radians)
...
def average_rmw(grib_dataset: xr.Dataset,
                cylindrical_grid_interpolator: cg.CylindricalGridInterpolator,
                u_surface_name: str, v_surface_name: str, radius_km: float,
                **_kwargs) -> float:
    LOGGER.info(
        "Starting calculation of average rmw u_surface_name:%s "
        "v_surface_name:%s radius_km:%f", u_surface_name, v_surface_name,
        radius_km)
    ...
    avg_rmw_km = post_cyl_avg_rmw(u_cyl, v_cyl, radius_km,
                                  lerp.rad_2d_km[0, :])

    LOGGER.info(
        "Finished calculation of average rmw:%f u_surface_name:%s "
        "v_surface_name:%s radius_km:%f", avg_rmw_km, u_surface_name,
        v_surface_name, radius_km)
    return avg_rmw_km


def post_cyl_avg_rmw(u_cyl: np.ndarray, v_cyl: np.ndarray, radius_km: float,
                     radii_1d_km: np.ndarray) -> float:
