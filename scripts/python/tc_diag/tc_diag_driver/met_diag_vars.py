...
def dataset_lookup(input_data: xr.Dataset, var_name: str,
                   **_kwargs: Dict[str, Any]) -> float:
...
def average_rmw(input_data: xr.Dataset, radii_1d: xr.DataArray,
                u_surface_name: str, v_surface_name: str, radius_km: float,
                **_kwargs: Dict[str, Any]) -> xr.DataArray:
    ...
    avg_rmw = diag_vars.post_cyl_avg_rmw(usurf.data.T, vsurf.data.T, radius_km,
                                         radii_1d.data)
...
def area_average(input_data: xr.Dataset,
                 radii_2d: np.ndarray,
                 min_radius_km: float,
                 max_radius_km: float,
                 var_name: str,
    ...
    avg = diag_vars.area_average(var.data, radii_2d, min_radius_km,
                                 max_radius_km)
...
def distance_to_land_lookup(lon: float, lat: float,
...
def storm_r_theta(
        track: pd.DataFrame, forecast_hour: int, init_time: dt.datetime,
        **_kwargs: Dict[str, Any]) -> Tuple[xr.DataArray, xr.DataArray]:
    storm_r, storm_theta = diag_vars.storm_r_theta(track, forecast_hour,
                                                   init_time)
...
def radial_and_tangential_area_average(
        input_data: xr.Dataset, radii_1d: xr.DataArray, theta_2d: np.ndarray,
    ...
    radial_avg = diag_vars.area_average(rad_azavg, radii_1d.values,
                                        min_radius_km, max_radius_km)
    tan_avg = diag_vars.area_average(tan_azavg, radii_1d.values, min_radius_km,
                                     max_radius_km)
    units = u.attrs["units"]
...
def _rad_tan(level_hPa: int, u: xr.DataArray, v: xr.DataArray,
             theta_2d: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
def div_vort(input_data: xr.Dataset, level_hPa: int, u_name: xr.DataArray,
             v_name: xr.DataArray, theta_2d: np.ndarray,
             radii_1d: xr.DataArray, radius_km: float,
             **_kwargs: Dict[str, Any]) -> Tuple[xr.DataArray, xr.DataArray]:
    u = input_data[u_name]
    v = input_data[v_name]
    rad_azavg, tan_azavg = _rad_tan(level_hPa, u, v, theta_2d)
    divergence, vorticity = diag_vars._div_vort(radii_1d, radius_km, rad_azavg,
                                                tan_azavg)
...
def shear(forecast_hour: int, results: fcresults.ForecastHourResults,
          u_name: str, v_name: str, bottom_hPa: int, top_hPa: int,
          **_kwargs: Dict[str, Any]) -> Tuple[xr.DataArray, xr.DataArray]:
    shear_r, shear_theta = diag_vars.shear(forecast_hour, results, u_name,
                                           v_name, bottom_hPa, top_hPa)
...
def temperature_gradient(forecast_hour: int,
                         results: fcresults.ForecastHourResults, u_name: str,
                         v_name: str, bottom_hPa: int, top_hPa: int,
                         lat: float, **_kwargs: Dict[str,
                                                     Any]) -> xr.DataArray:
    grad = diag_vars.temperature_gradient(forecast_hour, results, u_name,
                                          v_name, bottom_hPa, top_hPa, lat)
    grad *= TGRD_SCALE
